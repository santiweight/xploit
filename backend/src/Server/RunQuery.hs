...
import           Common.Server.Api              ( NodeQueryRequest(..)
                                                , NodeQueryResponse(..)
                                                )
...
getNode hands NodeQueryRequest {..} =
  let handResults =
        catMaybes $ flip (tryGetHandNode nodePath) includeHero <$> hands
      accRange = joinHandResults . fmap snd $ handResults
      getHandIfFilterMatched (hand, handResultRange) =
        case
            foldMap (Sum . length . filter (doesBetMatch nodeFilter))
                    handResultRange
          of
            0 -> Nothing
            _ -> Just hand
      handsMatchedFilter =
        take matchedHandsNum
          ...
          -- TODO fix ""
      getResultRange r = fromMaybe Map.empty $ r Map.!? ""
...
tryGetHandNode branch hand includeHero =
  fmap (hand, ) . listToMaybe . rights $ runIxBetsAsQuery hand
  -- TODO support hero
                                                          -- includeHero
                                                          branch
