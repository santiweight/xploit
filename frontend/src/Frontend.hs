...
whenLoaded loadedDs
           loadingWidget
           loadedWidget = do
    allLoadedD :: Dynamic t Bool <- pure . fmap and . sequence $ loadedDs
...
mainPane = do
  resD <- prerender ((constDyn "", constDyn acesHand) <$ text "My Prerender") $
          do
            rec
              ...
              let getMainRange :: Dynamic t (Range Hand [BetAction (Amount "USD")]
  ...
  where
    toHandFreqs :: ActionIx (Amount "USD") -> Range Hand [BetAction (Amount "USD")] -> Range Hand Double
    toHandFreqs ix mainRange =
      mainRange
    ...
    acesHand = mkPair Ace
    groupByShape
      :: Range Hand [BetAction (Amount "USD")]
      -> Range ShapedHand [BetAction (Amount "USD")]
    groupByShape rang = Range . Map.fromList $
          ffor listShapedHands $ \shapedHand ->
            let holdings = shapedHandToHands shapedHand
            in ( shapedHand
                , concatMap
...
--   -> Event t ()
--   -> m (Event t
--         (ReqResult () (Either (Either GameErrorBundle EvalErr) (Map.Map String (Range Hand [BetAction])))))
-- runQuery
--   :: MonadWidget t m
--   => Dynamic t (QParam [Char])
--   -> Event t ()
--   -> m (Event t
--         (ReqResult () (Map.Map String (Range Hand [BetAction (Amount "USD")]))))
-- loadHands
...
runQuery
  :: MonadWidget t m
  => Dynamic t (QParam [(Position, BetAction (IxRange (Amount "USD")))])
  -> Event t ()
  -> m (Event t
        (ReqResult () (Map.Map String (Range Hand [BetAction (Amount "USD")]))))
loadHands
...
--   => Dynamic t [(Position)]
--   -> Event t ()
--   -> m (Event t (Maybe (Map.Map String (Range Hand [BetAction (Amount "USD")]))))
